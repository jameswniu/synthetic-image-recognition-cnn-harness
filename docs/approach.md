...
Getting that second reader working took the evening's most useful failure. The textbook method matches distinctive points between two images, and it does not work here, because two software vendors rendering the same federal form use different fonts and different spacing. I measured it once during the build, and the best possible uniform stretch still left individual sections hundreds of pixels off, in both directions. Vendors reflow the sections independently. What survives is the order of the ruled lines, so the alignment matches sequences rather than stretching coordinates.
...
A small trained model rides alongside as a second opinion on the mark. It is deliberately tiny, 23,381 weights, because with four real pages any extra capacity would be spent memorizing my own generator. It can raise confidence when it agrees and flag a box when it does not, and it can never overrule the deterministic read.

## Where the AI sits, and what that costs

Nothing in the reading path is an AI model. That path is ordinary image processing, a median of 273 ms per page on one processor core measured across 61 pages, no model anywhere in it, and identical output on identical input forever. Through the HTTP service, 120 requests of the photographed page come back at a median of 50 ms. That last property is not aesthetic. When someone asks in twelve months why a file recorded a particular answer, the system can be re-run to show them.
